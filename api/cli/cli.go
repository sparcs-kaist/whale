...
const (
	errInvalidEnpointProtocol  = whale.Error("Invalid endpoint protocol: Whale only supports unix:// or tcp://")
	errSocketNotFound          = whale.Error("Unable to locate Unix socket")
	errEndpointsFileNotFound   = whale.Error("Unable to locate external endpoints file")
	errInvalidSyncInterval     = whale.Error("Invalid synchronization interval")
	errEndpointExcludeExternal = whale.Error("Cannot use the -H flag mutually with --external-endpoints")
)

// ParseFlags parse the CLI flags and return a whale.Flags struct
func (*Service) ParseFlags(version string) (*whale.CLIFlags, error) {
	kingpin.Version(version)

	flags := &whale.CLIFlags{
		Endpoint:          kingpin.Flag("host", "Dockerd endpoint").Short('H').String(),
		Logo:              kingpin.Flag("logo", "URL for the logo displayed in the UI").String(),
		Labels:            pairs(kingpin.Flag("hide-label", "Hide containers with a specific label in the UI").Short('l')),
		ExternalEndpoints: kingpin.Flag("external-endpoints", "Path to a file defining available endpoints").String(),
		SyncInterval:      kingpin.Flag("sync-interval", "Duration between each synchronization via the external endpoints source").Default(defaultSyncInterval).String(),
		Addr:              kingpin.Flag("bind", "Address and port to serve Whale").Default(defaultBindAddress).Short('p').String(),
		...
		SSOID:             kingpin.Flag("ssoid", "SPARCS SSO ID").Default(defaultSSOID).String(),
		SSOKey:            kingpin.Flag("ssokey", "SPRACS SSO Key").Default(defaultSSOKey).String(),
	}
...
func (*Service) ValidateFlags(flags *whale.CLIFlags) error {
